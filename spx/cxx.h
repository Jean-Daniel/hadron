 ...
 */
/*!
 @abstract c++ standardization. Provide substitution for missing C++ features.
*/
...
// FIXME: is there a libstdc++ version that provide <type_traits> ?
#if __has_include(<type_traits>) || defined(_MSC_VER) || (defined(__GLIBCXX__) && __GLIBCXX__ >= 20080905)
  #include <type_traits>
  ...
  namespace std {
    ...
    namespace std {
      // enable_if<bool, type>
      template <bool, class _Tp = void> struct SPX_VISIBLE enable_if {};
      template <class _Tp> struct SPX_VISIBLE enable_if<true, _Tp> {typedef _Tp type;};

      // conditional<bool, if, then>
      template <bool _B, class _If, class _Then>
      struct SPX_VISIBLE conditional {typedef _If type;};
      template <class _If, class _Then>
      struct SPX_VISIBLE conditional<false, _If, _Then> {typedef _Then type;};
